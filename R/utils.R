...
check_request_status <- function(request_obj) {
  if (request_obj$status_code == 401) {
    message(paste0("There was a 401 error.",
                   "Do you need to refresh your access token?"))
    stop_quietly()
  } else if (request_obj$status_code != 200) {
    message(paste0("There was an authentication error: ",
...
check_for_faulty_xml <- function(request_obj) {
  if (length(grep("<FaultString>", httr::content(request_obj, "text",
